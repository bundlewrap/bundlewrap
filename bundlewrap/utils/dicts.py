...
VALID_STATE_DICT_TYPES = tuple(TYPE_DIFF_NORMALIZE.keys()) + (str,)
...
def hash_state_dict(state_dict):
    ...
    """
    return sha256(state_dict_to_json(state_dict).encode('utf-8')).hexdigest()
...
def state_dict_to_json(state_dict, pretty=False):
    """
    Returns a canonical JSON representation of the given state_dict.
    """
...
def validate_state_dict(state_dict):
    ...
    for key, value in state_dict.items():
        if not isinstance(force_text(key), str):
            raise ValueError(_("non-text state_dict key: {}").format(key))

        if not isinstance(value, VALID_STATE_DICT_TYPES) and value is not None:
            raise ValueError(_(
                "invalid state_dict value for key '{k}': {v}"
            ).format(
                k=key,
                v=repr(value),
            ))
        ...
        if isinstance(value, (list, tuple)):
            for index, element in enumerate(value):
                if not isinstance(element, VALID_STATE_DICT_TYPES) and element is not None:
                    raise ValueError(_(
                        "invalid element #{i} in state_dict key '{k}': {e}"
                    ).format(
                        e=repr(element),
                        i=index,
                        k=key,
