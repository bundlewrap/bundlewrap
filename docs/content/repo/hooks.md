...
**`secret_key_use(repo, key, **kwargs)`**

Called when a key from [`.secrets.cfg`](../guide/secrets.md) is used, i.e. when a Fault generated by `repo.vault` or `node.vault` is resolved. It runs at most once at a time per key: other threads resolving a Fault with the same key while it is running skip it, so it cannot serve as a reliable guard for every single use of a key.

`repo` The current repository (instance of `bundlewrap.repo.Repository`).

`key` The secret key that's being used

`node` The node whose `node.vault` is being used (instance of `bundlewrap.node.Node`). Only present for `node.vault`, so use `kwargs.get('node')`.

To skip a secret:

```
from bundlewrap.exceptions import FaultUnavailable
raise FaultUnavailable("reason goes here")
```
