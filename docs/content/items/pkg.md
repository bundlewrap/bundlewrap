...
Usually, the `apt` bundle would be added automatically to the nodes via a [group](group.md) so that it does not need to be added
manually to every node. Now, the conflict resolution is handled by the metadata dict which will warn, when metadata clashes
between bundles, but now you are free to implement whichever resolution algorithm you want in your package management bundle.
